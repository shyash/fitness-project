...
function drawSkeleton(parts) {
    ...
    for (let i = 5; i < 14; i+=2) {
        line(parts[i].position.x,parts[i].position.y,parts[i+2].position.x,parts[i+2].position.y)
        line(parts[i+1].position.x,parts[i+1].position.y,parts[i+3].position.x,parts[i+3].position.y)
        if (i == 7) i= 9
        if (i==5 || i ==11)  line(parts[i].position.x,parts[i].position.y,parts[i+1].position.x,parts[i+1].position.y)
        if (i==5 || i == 6){
            line(parts[i].position.x,parts[i].position.y,parts[i+6].position.x,parts[i+6].position.y)
            line(parts[i+1].position.x,parts[i+1].position.y,parts[i+7].position.x,parts[i+7].position.y)
        } 
    }
